...
st.subheader("Jumlah penyewaan sepeda berdasarkan musim", help=None)

colors1 = ["burlywood", "burlywood", "burlywood", "darkorange"]
colors2 = ["peachpuff", "peachpuff", "peachpuff", "salmon"]

fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(35,15))

sns.barplot(y="count", x="season", hue="season", data=days_df.sort_values(by="count", ascending=True), 
            palette=colors1, errorbar=None, ax=ax[0])
ax[0].set_title("Grafik Penyewaan Sepeda Berdasarkan Musim Tertinggi", fontsize=30)
ax[0].set_xlabel("Musim", fontsize=30)
ax[0].set_ylabel("Jumlah", fontsize=30)
ax[0].tick_params(axis='both', labelsize=27)

sns.barplot(y="count", x="season", hue="season", data=days_df.sort_values(by="count", ascending=False), 
            palette=colors2, errorbar=None, ax=ax[1])
ax[1].set_title("Grafik Penyewaan Sepeda Berdasarkan Musim Terendah", fontsize=30)
ax[1].set_xlabel("Musim", fontsize=30)
ax[1].set_ylabel("Jumlah penyewaan sepeda", fontsize=30)
ax[1].tick_params(axis='both', labelsize=27)
...
sns.barplot(
        ...
        x ="workingday",
        hue ="workingday",
        data = days_df.sort_values(by="workingday", ascending=False),
        palette = colors,
        errorbar = None,
        ax = ax
    )
