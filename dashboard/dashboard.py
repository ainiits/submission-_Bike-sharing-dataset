...
def create_total_registered_df(day_df):
   registered_df = day_df.groupby(by="date_day")["registered"].sum().reset_index()
...
with st.sidebar:
    ...
    start_date, end_date = st.date_input("Rentang Waktu", [min_date, max_date], min_value=min_date, max_value=max_date, key="date_range")
...
# Filter 
filtered_df_days = days_df[(days_df["date_day"] >= start_date) &
                           (days_df["date_day"] <= end_date) &
                           (days_df["season"].isin(selected_season)) &
                           (days_df["weathersit"].isin(selected_weather))]

filtered_df_hours = hours_df[(hours_df["date_day"] >= start_date) &
                             (hours_df["date_day"] <= end_date) &
                             (hours_df["season"].isin(selected_season)) & 
                             (hours_df["weathersit"].isin(selected_weather))]
...
filtered_df_hours["kategori_waktu"] = pd.Categorical(filtered_df_hours["kategori_waktu"], ordered=True)
filtered_df_hours["date_day"] = pd.to_datetime(filtered_df_hours["date_day"])
filtered_df_hours["month"] = filtered_df_hours["date_day"].dt.strftime('%Y-%m')
waktu_grouped = filtered_df_hours.groupby(["month", "kategori_waktu"], observed=True)["count"].sum().reset_index()
...
monthly_count = days_df.groupby("date_day")["count"].sum()
fig, ax = plt.subplots(figsize=(20,10))
ax.scatter(monthly_count.index, monthly_count.values, color="red", s=20, marker='o')
ax.plot(monthly_count.index, monthly_count.values, color="skyblue")
ax.set_xlabel("")
ax.set_ylabel("Jumlah Penyewaan Sepeda", fontsize=20)
ax.set_title("Grafik Jumlah Penyewa Sepeda Tahun 2011-2012", fontsize=20)
...
hourly_count = hours_df["count"].groupby(hours_df["hours"]).sum()
